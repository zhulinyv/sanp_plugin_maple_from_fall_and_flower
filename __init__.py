...
path = "./plugins/webui/sanp_plugin_maple_from_fall_and_flower"
...
def seach(inp, num, sf="search", yin="yinyin", yinli=[], start=0):
    ...
    if len(inp) != 0:
        ...
        inputbehind = [
            it for it in input if inp not in it.get("tags") and inp not in it.get("chin").replace("\\n", " ")
        ]
        inputfront = sorted(
            inputfront,
...
def radio_to_out(li, rad, sab, cho="one_tags"):
    ...
    for it in yuan:
        ...
        if (
            ...
            and "【特殊符号：" not in it
            and (ind == 0 or set(list(yuan[ind + 1].split("$")[0])) != set(list(yuan[ind - 1].split("$")[0])))
        ):
            qian = "["
        else:
            qian = ""
        if "【特殊符号：" in it:
            if (
                ind == 0
                or ind == len(yuan) - 1
                or set(list(yuan[ind + 1].split("【")[0].split("$")[0])) == set(list(word))
            ):
                add = word + " "
                hou = ""
            ...
            add = (
                ...
                + (
                    ", "
                    if word in sab
                    and (ind != len(yuan) - 1 or "最后一个去逗号" not in getItem("set_little", []))
...
def check_to_sub(check, radio, li, su, bas, sab, little, clear=False):
    ...
    ssab = [
        it.split("【")[0].replace("?(&&", "").replace("?[&&", "").replace("?{&&", "") for it in check if it[0] == "?"
    ]
...
def zhou_to_check(nname):
    ...
    if yuan.get(nname):
        yuan = yuan.get(nname)
        check = [
            (it if isinstance(it, str) else ((tags[it[1]], it[2]) if it[0] == "search" else (ddict[it[1]], it[2])))
            for it in yuan[2]
        ]
        bas = [
            (it if isinstance(it, str) else (tags[it[1]]["tags"] if it[0] == "search" else ddict[it[1]]["tags"]))
            for it in yuan[1]
        ]
        sab = [
            (it if isinstance(it, str) else (tags[it[1]]["tags"] if it[0] == "search" else ddict[it[1]]["tags"]))
        ...
        check = [
            ("?" if it.split("【")[0] not in sab else "") + ("(" if it.split("【")[0] in bas else "{") + "&&" + it
            for it in check
        ]
    ...
    out, bas, sab = radio_to_out(
        ...
        rad=[
            (
                it
                if isinstance(it, str)
                else (tags[it[1]].get("tags") if it[0] == "search" else ddict[it[1]].get("tags"))
            )
            for it in yuan[1]
        ],
        sab=(
...
def cheese_to_all(warn, cheese):
    ...
    img = [
        ...
        for it in os.listdir(path + "/images/magic" + "@".join(re.findall(r"\d", warn)) + "/")
...
def image_appear(warning, name, num):
    ...
    img = [
        path + "/images/magic" + "@".join(re.findall(r"\d", warning)) + "/" + it
        for it in os.listdir(path + "/images/magic" + "@".join(re.findall(r"\d", warning)) + "/")
        if it.split("@")[0] == name
    ]
    return img[num]
...
def zhou_to_cun(name, li, rad, sab):
    ...
    yuan = [
        ("?" if it.split("【")[0] not in sab else "") + ("(" if it.split("【")[0] in rad else "{") + "&&" + it
        for it in yuan
    ]
...
def change_tabs(change, warn, cheese, goodcheck, badcheck):
    ...
    for it in change:
        if it[3] == "改":
            yester = it[4:].split("为")[0]  # noqa: F841
            now = it[4:].split("为")[1]  # noqa: F841
            tagskind = True if it[1] == "正" else False
        elif it[3:5] == "消掉":
            xiao = it[5:]  # noqa: F841
            tagskind = True if it[1] == "正" else False  # noqa: F841
        elif it[3] == "加":
            addadd = it[4:].split("在")[0]  # noqa: F841
            qian = it[4:].split("在")  # noqa: F841
    ...
    with gr.Tab("maple的tag选择器"):
        with gr.Column():
            ...
            (
                ...
                ccansamples,
                ccancfg,
            ) = lit_to_bassab(getItem("set_little", []), [], [], getItem("set_dro", li[0]), *canmo)
            with gr.Row():
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    txt2imgbutton = gr.Button(value="转到文生图页面并复制参数")  # noqa: F841
                    img2imgbutton = gr.Button(value="转到图生图页面并复制参数")  # noqa: F841
        with gr.Tab(label="单个咒语书柜"):
            ...
            with gr.Row():
                with gr.Column(scale=9):
                    check = gr.CheckboxGroup(
                        choices=[it[0] for it in seach("", getItem("set_num", 100))],
                        label="此处是单个咒语搜索结果",
                        value=[],
                    )
                with gr.Column(scale=1):
            ...
            text.change(
                fn=lambda *it: (text_to_check(*it) if "按下enter键时搜索才生效" not in it[2] else (it[3], it[3])),
            ...
            with gr.Tab(label="元素法典卷轴"):
                ...
                for item in mag:
                    with gr.Tab(label=item):
                        ...
                        with gr.Row():
                            ...
                            with gr.Column(scale=1):
                                img = [
                                    path + "/images/magic" + "@".join(re.findall(r"\d", item)) + "/" + it
                                    for it in os.listdir(
                                        path + "/images/magic" + "@".join(re.findall(r"\d", item)) + "/"
                                    )
                ...
                for item in mag_item:
                    ...
                    mag_item[item].imagecho.change(
                        fn=image_appear,
                        inputs=[mag_item[item].warn, mag_item[item].cheese, mag_item[item].imagecho],
                        outputs=mag_item[item].image,
                    )
                    mag_item[item].goodcheck.change(
                        ...
                        inputs=[mag_item[item].goodcheck, mag_item[item].warn, mag_item[item].cheese],
                    ...
                    mag_item[item].badcheck.change(
                        ...
                        inputs=[mag_item[item].badcheck, mag_item[item].warn, mag_item[item].cheese],
                        outputs=mag_item[item].bad,
                    )
                    mag_item[item].goodsub.click(
                        fn=check_to_sub,
                        inputs=[mag_item[item].good, radio, dro, mag_item[item].goodsub, bas, sab, little],
                        outputs=[radio, radio, out, bas, bas, dro, sab, sab],
                    )
                    mag_item[item].badsub.click(
                        fn=check_to_sub,
                        inputs=[mag_item[item].bad, radio, dro, mag_item[item].badsub, bas, sab, little],
                        outputs=[radio, radio, out, bas, bas, dro, sab, sab],
                    )
                    mag_item[item].goodsubding.click(
                        fn=check_to_sub,
                        inputs=[mag_item[item].good, radio, dro, mag_item[item].goodsubding, bas, sab, little],
                        outputs=[radio, radio, out, bas, bas, dro, sab, sab],
                    )
                    mag_item[item].badsubding.click(
                        fn=check_to_sub,
                        inputs=[mag_item[item].bad, radio, dro, mag_item[item].badsubding, bas, sab, little],
                        outputs=[radio, radio, out, bas, bas, dro, sab, sab],
                    )
        if dictperfect:
            with gr.Tab(label="遗失魔法碎片"):
                ...
                yyin.change(
                    fn=lambda yin, text, num, check: ttext_to_check(text, num, check, yin, eng=""),
    ...
    if benben == 2:
        ...
        for it in zhoucun:
            ...
            if len(item) == 3:
                item.append(
                    [
                        (it if isinstance(it, list) else re.sub(r"\??[\(\[\{]?&{0,2}", "", it.split("【")[0]))
                        for it in item[2]
                    ]
                )
